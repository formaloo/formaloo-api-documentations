...
Use `PUT` when replacing the editable form configuration. For smaller edits, prefer `PATCH`.
...
Retrieve the form before replacing it and preserve fields that are not intentionally changing. Sending `logic` replaces the saved logic array and should use exact field, choice, variable, template, webhook, and success-page slugs.
